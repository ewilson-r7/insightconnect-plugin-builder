...
> repairing.
>
> **Revision note, 26.3 (export gate and preview fidelity bugfix, clause 2.6).**
> 26.3 required the Quality_Gate to exclude generated files but said nothing about
> the other checks, and each subsystem answered the question for itself: the
> Code_Validator's lint stage had no notion of it at all, so an export was blocked
> by fourteen findings in generated `__init__.py` and `schema.py` files -- real,
> correctly located, and unfixable by their audience. The clause now names a single
> definition that every check consumes. The tradeoff, accepted: a genuine defect
> inside a generated file goes unreported by these checks, which is tolerable
> because such a defect is a defect in the CLI's templates and could not be fixed
> in the plugin.
...
3. THE Quality_Gate SHALL exclude files generated by the Insight_Plugin_CLI from its Findings, because the Agent_Rulebook forbids editing them and a Finding against one is not actionable; the set of generated files SHALL be determined by a single definition, consumed by the Quality_Gate, the Code_Validator's lint stage, and the packaging exclusion alike, so the three cannot disagree about which files a plugin's author may be judged on.
...
>
> **Revision note, 27.1's lint and formatting clauses (same bugfix, clauses 2.7,
> 2.8).** The clauses named hand-written code for the linter but not for the
> formatter, and named neither the profile nor the width. Both mattered in practice:
> the `formatted` condition read unmet on the strength of generated `schema.py` and
> `setup.py` files, and a `would-reformat` finding raised at 120 columns was
> indistinguishable from one raised at the formatter's own narrower default. The
> clauses now state that both apply to hand-written code only, and that the bar is
> the profile and width the plugins repository itself applies.

#### Acceptance Criteria
1. THE Plugin_Builder SHALL treat a plugin as complete only when all of the following hold: the Insight_Plugin_CLI validate operation passes; the Agent_Rulebook's linter, run under the profile that repository applies and at the width it formats to, reports no findings against hand-written code; formatting matches that width, judged over hand-written code only; every hand-written Python file parses; the plugin exposes an API client with centralized request handling and per-action methods, whose status-to-exception map is either defined in or imported into the client module from within the plugin package; the connection's connect and test operations are implemented rather than stubbed; unit tests covering each action pass; statement coverage of the plugin package meets the configured minimum; and a dependency manifest exists.
