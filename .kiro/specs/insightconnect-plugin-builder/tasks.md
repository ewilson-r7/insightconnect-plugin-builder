...
- [x] 39. Ask for documentation before implementing a vendor plugin without it
  - **Decision taken: the interpretation layer makes the call.** Whether a request means to call somebody's HTTP API is a question about intent, and the interpreter is the only component that reads the user's words, so `vendor_api` joins its response schema. The alternatives were a spec-shape heuristic (still inference, and only available after the connection exists) or an explicit question at creation time (deterministic, but adds a prompt to every plugin). This is the one input to the definition-of-done surface that is a judgment rather than a check, and the prompt names *both* ways it can be wrong so the model is not pushed toward answering "vendor" for everything.
  - The request rides the existing clarification path, so it leaves the draft untouched like any other clarification (Req 1.5), and it is asked only when the turn would implement code, a vendor was identified, nothing has been supplied or already stored, and the user has not said to go ahead. Each of those narrows a false alarm, which matters because an operator who learns to dismiss this question gets nothing from it.
  - The question is not a dead end: it names all three routes and says how to proceed anyway. Proceeding is recorded in the tree (`implemented_without_reference` in the provenance record), not just in the session, so a plugin built on guessed endpoints still says so when reopened tomorrow (Req 28.13).
  - New Definition_Of_Done condition `reference_material`, with three outcomes rather than two: met when documentation was stored, unmet only when the tree records that implementation deliberately went ahead without it, and **unverified** when nothing was recorded either way -- because a plugin that encodes base64 needs no vendor documentation, and reporting it as unmet would be a false alarm.
  - _Requirements: 28.12, 28.13, 27.1, 27.5_
...
- [x] 37. Reconcile Requirement 8 with the checks that actually run
  ...
  - **Requirement 8 revised** to say that the four stages are the export gate and only the export gate, rather than reading as the whole validation surface. The wider set -- parse, formatting, lint, unit tests, coverage, spec completeness -- belongs to Requirements 26 and 27 and is advisory.
  - **The validate stage now runs the toolchain's validators minus a named exclusion list** (new Req 8.9-8.12, `integrations/plugin_validate.py`). This was not a relaxation but a repair: `insight-plugin validate` runs every validator, one of which fetches the plugin's previous spec from the `insightconnect-plugins` git remote and locates it by splitting the path on `/plugins/`. Outside a clone it raises `InvalidGitRepositoryError`, then a `TypeError` while handling it, and the process dies -- so validators after it never run, and because `validate()` prints its failures only after the loop, the failures already collected are never reported either.
